        ...
        <div class="popupEntriesDiv relatedContents">
            ...
            <div class="popupEntriesTableContainer {if $MODULE eq 'EmailTemplates'} emailTemplatesPopupTableContainer{/if}">
                ...
                {foreach item=LISTVIEW_ENTRY from=$LISTVIEW_ENTRIES name=popupListView}
                    ...
                    <tr class="listViewScEntries" data-id="{$LISTVIEW_ENTRY->getId()}" data-selling_price="{$LISTVIEW_ENTRY->get('selling_price')}"  {if $MODULE eq 'EmailTemplates'} data-consultantname="{$RECORD_DATA['consultantname']}"  data-name="{$RECORD_DATA['subject']}" data-info="{$LISTVIEW_ENTRY->get('body')}" {else} data-name="{$LISTVIEW_ENTRY->getName()}" data-info='{Vtiger_Util_Helper::toSafeHTML(ZEND_JSON::encode($LISTVIEW_ENTRY->getRawData()))}' {/if}
