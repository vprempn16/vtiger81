...
{strip}
	...
	{assign var="IS_ADMIN" value=$CURRENT_USER_MODEL->isAdminUser()}
					...
					<table class="table table-borderless">
						<tr>
							...
							{foreach key=FIELD_NAME item=FIELD_MODEL from=$BLOCK_FIELDS name=blockfields}
                                    {if $FIELD_NAME eq 'consultantrole' }
                                        {continue}
									{ELSE}
										{IF $IS_ADMIN neq true AND ( $FIELD_NAME eq 'approvalstatus' OR $FIELD_NAME eq 'scstatus' ) }
											{continue}
										{/IF}
                                    {/if}
