...
//		this.calculateGrandTotal();
	...
	mapResultsToFields: function(parentRow,responseData){
		...
		for(var id in responseData){
			...
			var rowNum = parentRow.attr('data-row-num');
			var listPrize = recordData.listprice;
			/*if(parentRow.find("input[name='atomsorgprice']").length){
			parentRow.find("input[name='atomsorgprice']").remove();
			parentRow.find('.atompricehike-td').find("input[name='atompricehike']").remove();
			var atomsorgprice = "atomsorgprice"+rowNum;
			var atompricehike = "atompricehike"+rowNum;
			parentRow.find('.atompricehike-td').append("<input type='text' class='smallInputBox inputElement' name='"+atompricehike+"' value='0'>");
			parentRow.append("<input type='hidden' name='"+atomsorgprice+"' value='" + listPrize + "'>");
	    		}*/
			//new 
			var atomsorgprice = "atomsorgprice" + rowNum;
			var atompricehike = "atompricehike" + rowNum;
			var global_pricehike = jQuery('#global_pricehike').val();
			parentRow.find("input[name='" + atomsorgprice + "']").remove();
			parentRow.find("input[name='" + atompricehike + "']").remove();
			parentRow.find('.atompricehike-td').find("input[name='atompricehike']").remove();

			parentRow.find('.atompricehike-td').append(
				"<input type='text' class='smallInputBox inputElement' name='" + atompricehike + "' value='"+global_pricehike+"'>"
			);
			parentRow.append(
				"<input type='hidden' name='" + atomsorgprice + "'>"
			);
			parentRow.find("input[name='atomsorgprice']").remove();
			parentRow.find("input[name='" + atomsorgprice + "']").val(listPrize);

			//parentRow.find(".atompricehike-td").find("input[name='atompricehike"+rowNum+"']").trigger('click');
			if(parentRow.find('.atom-hike-preview').length){
				var preview = parentRow.find('.atom-hike-preview');
				var newPrice = listPrize + (listPrize * 0 / 100);
				newPrice = newPrice.toFixed(2);
				var previewText = 'Markup = 0 + Price =' + listPrize + ' = ' + newPrice;
				var sellingPriceInput = parentRow.find("input[name='listPrice" + rowNum + "']");
				if(preview.length ){
					preview.remove();
					sellingPriceInput.before('<div style="font-size: 11px; color: #888;"><span class="atom-hike-preview">'+previewText+'</span></div>');
				}
				//jQuery('#global_pricehike').trigger('input');
				parentRow.find(".atompricehike-td").find("input[name='atompricehike"+rowNum+"']").trigger('input');
			}
            var consultantname = "consultantname" + rowNum; //ISSUE266 Start
