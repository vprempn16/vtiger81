...
class Settings_AtomMSmtpWorkFlow_SaveLicense_Action extends  Settings_Vtiger_Basic_Action {
    public function process(Vtiger_Request $request) {
        global $current_user,$adb;
        $response = new Vtiger_Response();
        $data = $request->get('formData');
        parse_str($data,$formData);
        $license_key = $formData['msmtpworkflow_license_key'];
        $result = false;
        $message = "Failed";
        ...
        $action = 'validate';
        if($license_key == ''){
            $license_key = $this->getValue('msmtpworkflow_license_key');
        }
        if($license_key == ''){
        ...
        if($license_key != ''){
            ...
            if($checkApiKey['status']){
                if($formData['msmtpworkflow_license_key'] == ''){
                    $formData['msmtpworkflow_license_key'] = $license_key;
                }
                $formData['msmtpworkflow_license_key'] = Vtiger_Functions::toProtectedText($formData['msmtpworkflow_license_key']);
                foreach($formData as $key => $value){
                    if($key == 'msmtpworkflow_license_key'){
