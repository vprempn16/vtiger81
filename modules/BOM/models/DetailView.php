...
class BOM_DetailView_Model extends Inventory_DetailView_Model {
	...
	public function getDetailViewLinks($linkParams) {
		...
		if($currentUserModel->hasModuleActionPermission($purchaseOrderModuleModel->getId(), 'CreateView')) {
			...
			$generatePOActionLink = array(
				'linktype' => 'LISTVIEWBASIC',
                                'linklabel' => vtranslate('LBL_GENERATE').' '.vtranslate($purchaseOrderModuleModel->getSingularLabelKey(), 'PurchaseOrder'),
                                'linkurl' => 'javascript:BOMHeader_Js.generatePOPopup("'.$recordModel->getPopupUrl().'")',
                                'linkicon' => ''
			);
			$linkModelList['DETAILVIEW'][] = Vtiger_Link_Model::getInstanceFromValues($generatePOActionLink);
		}
